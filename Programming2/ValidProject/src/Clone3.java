public class Clone3 {
    ...
    public static void main(String[] args) {
        ...
        String myEmail = "weeewoo@weewoo.o9j";
    ...
    public static String isUsername(String input) {
        ...
        if (input == null || input.length() < 1 || input.length() > 7 || reSpecial(input) || !isSpecialChar(input.charAt(0), false)) {
            ...
            for (int i = 1; i < input.length(); i++) {
                char check = input.charAt(i);
                // no alphanumeric character or character is neither a allowed domainchar nor !
    ...
    public static boolean reChar(String input) {
        ...
        for (int i = 0; i < input.length(); i++) { //iterates through each char
            char check = input.charAt(i);
            if (i == 0) {    //if its the first one skip rest of iteration
                continue;
            } else {
